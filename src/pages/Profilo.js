...
export default class Profilo_page extends React.Component{
    constructor(props){
        super(props);
        this.state={
            utente: {},
    ...
    init(){
        ...
        .then(res3 => res3.json())
        .then(res3 => arr.push(res3))
        .then(()=>console.log(arr))
        .then(()=>fetch(window.server_url+'sale'))
        .then(res4 => res4.json())
        .then(res4 => arr.push(res4))
        .then(console.log(arr))
        .then(()=> {
    ...
    show_abbonamenti(){
        //console.log(this.state.abbonamenti);
        var array=[];
            ...
            <Container fluid >
                <Row className='cont_img_Utenti'>
                    ...
                    <Col className='d-flex justify-content-center'>
                        <h1 id='text_shadow' className='text-white'>{this.state.utente.nome} {this.state.utente.cognome}</h1>
                    </Col>
                    <Col xs={12}></Col>
                </Row>
                ...
                <Col xs={6}>
                    ...
                    <Alert variant='warning' hidden={window.logged}>
                            Devi prima accedere per poter visualizzare questa pagina!
                            <Button variant='outline-dark' className='my-2' as={Link} to='/Accedi'>Torna alla pagina accedi</Button>
                    </Alert>
                </Col>
                <Col></Col>
                </Row>
                <Row className='d-flex my-2'>
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control placeholder={this.state.utente.password} disabled={!this.state.modifying} onChange={(event) => this.setState({passw_m: event.target.value})} value={this.state.passw_m}/>
                    </Form.Group>
                    <Form.Group className="mb-3">
